...
eventRouter.post('/', async (request, response) => {
  ...
  try {
    ...
    if (body.eventTypes === undefined || body.eventTypes === '' || body.eventTypes.lenght() === 0) {
      response.status(400).send({ error: 'event type missing' })
    }
    if (body.locations === undefined || body.locations === '' || body.locations.lenght() === 0) {
      response.status(400).send({ error: 'location missing' })
    }
    if (body.organizers === undefined || body.organizers === '' || body.organizers.lenght() === 0) {
